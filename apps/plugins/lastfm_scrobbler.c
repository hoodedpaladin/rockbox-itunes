                        ...
                        struct cache_entry *entry, int *trk_info_len)
{
    ...
    int ret = rb->snprintf(entry->buf,
                   SCROBBLER_CACHE_LEN,
                   "%s"SEP"%s%n"SEP"%ld"SEP"%lu/%lu"EOL"",
                   str_chk_valid(id->itid, ""),
                   str_chk_valid(id->path, ""),
...
static void scrobbler_add_to_cache(const struct mp3entry *id)
{
    ...
    else if (ret < 0 || (size_t) ret >= SCROBBLER_CACHE_LEN)
    {
...
static void track_change_event(unsigned short id, __attribute__((unused)) void *ev_data)
{
    (void)id;
    logf("%s", __func__);
    ...
    // Nah don't worry, we log real playtime now
...
static void track_finish_event(unsigned short id, void *ev_data)
{
    ...
    // Still scrobble on rewind
    if (gCache.pending && (te->flags & TEF_CURRENT))
    {
        gCache.pending = false;
